...
# Modifications:
# Copyright (c) [2025] [QianMuXiao]
# Modified version of MONAI's AutoencoderKL for MICCAI2025-FDA-VAE.
# Original code: https://github.com/Project-MONAI/GenerativeModels/blob/main/generative/networks/nets/autoencoderkl.py
# 
# Modifications include:
# - Addition of dual decoder architecture for CT-MR translation
# - Implementation of distribution flipping in latent space
# - Modified forward and reconstruction methods to handle modality-specific operations
# - Added input modality labels ('ct' or 'mr') for directed translation
# - Separate decoders and quantization blocks for each modality
# - Distribution flipping mechanism for cross-modality synthesis
...
class AutoencoderKL(nn.Module):
    ...
    def flip_distribution(self, z_mu: torch.Tensor, z_sigma: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        """
        Flips the distribution based on standard normal distribution symmetry.

        Args:
            z_mu: Mean vector of the latent space
            z_sigma: Standard deviation vector of the latent space

        Returns:
            Flipped mean and standard deviation vectors
        """
        flipped_z_mu = -z_mu
        flipped_z_sigma = z_sigma  # Standard deviation remains the same

        return flipped_z_mu, flipped_z_sigma
    ...
    def decode_ct(self, z: torch.Tensor) -> torch.Tensor:
        """
    ...
    def decode_mr(self, z: torch.Tensor) -> torch.Tensor:
        """
        Based on a latent space sample, forwards it through the Decoder.
        ...
        Returns:
            decoded image tensor
        """
        z2 = self.post_quant_conv_2(z)
        if self.use_checkpointing:
            dec2 = torch.utils.checkpoint.checkpoint(self.decoder_2, z2, use_reentrant=False)
        else:
            dec2 = self.decoder_2(z2)
        return dec2
    
    # def val_reconstruct(self, x: torch.Tensor) -> torch.Tensor:
    #     z_mu, _ = self.encode(x)
    #     reconstruction_ct = self.decode_ct(z_mu)
    #     reconstruction_mr = self.decode_mr(z_mu)
    #     return reconstruction_ct, reconstruction_mr

    # def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    #     z_mu, z_sigma = self.encode(x)
    #     z = self.sampling(z_mu, z_sigma)
    #     reconstruction = self.decode_ct(z)
    #     reconstruction2 = self.decode_mr(z)
    #     return reconstruction, reconstruction2, z_mu, z_sigma
    ...
    def val_reconstruct(self, x: torch.Tensor, label: str) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if label == 'ct':
            ...
            # Flip the distribution for MR
            flipped_z_mu, _ = self.flip_distribution(z_mu, z_sigma)
            reconstruction_mr = self.decode_mr(flipped_z_mu)
            
        elif label == 'mr':
            reconstruction_mr = self.decode_mr(z_mu)
            
            # Flip the distribution for CT
            flipped_z_mu, _ = self.flip_distribution(z_mu, z_sigma)
            reconstruction_ct = self.decode_ct(flipped_z_mu)
            
        else:
            raise ValueError("Label must be either 'ct' or 'mr'")
    ...
    def forward(self, x: torch.Tensor, label: str) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if label == 'ct':
            z_ct = self.sampling(z_mu, z_sigma)
            reconstruction_ct = self.decode_ct(z_ct)
            
            # Flip the distribution for MR
            flipped_z_mu, flipped_z_sigma = self.flip_distribution(z_mu, z_sigma)
            z_mr = self.sampling(flipped_z_mu, flipped_z_sigma)
            reconstruction_mr = self.decode_mr(z_mr)
            
        elif label == 'mr':
            z_mr = self.sampling(z_mu, z_sigma)
            reconstruction_mr = self.decode_mr(z_mr)
            
            # Flip the distribution for CT
            flipped_z_mu, flipped_z_sigma = self.flip_distribution(z_mu, z_sigma)
            z_ct = self.sampling(flipped_z_mu, flipped_z_sigma)
            reconstruction_ct = self.decode_ct(z_ct)
            
        else:
            raise ValueError("Label must be either 'ct' or 'mr'")
        
        return reconstruction_ct, reconstruction_mr, z_mu, z_sigma
    ...
    def decode_stage_2_outputs(self, z: torch.Tensor) -> torch.Tensor:
        image = self.decode_ct(z)
        image2 = self.decode_mr(z)
        return image, image2
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        ...
        mu, sigma = autoencoder.encode(x)
        z = autoencoder.sampling(mu, sigma)
        out = autoencoder.decode_ct(z)
